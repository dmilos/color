            ...
            quadrature( ::color::_internal::model< category_name > const& color_parameter  )
             {
              ...
              float_name value =
                 + 0.211 * ( color_parameter.template get<0>() - trait_type::template minimum<0>() ) / float_name( trait_type::template range<0>() )
                 - 0.523 * ( color_parameter.template get<1>() - trait_type::template minimum<1>() ) / float_name( trait_type::template range<1>() )
                 + 0.312 * ( color_parameter.template get<2>() - trait_type::template minimum<2>() ) / float_name( trait_type::template range<2>() )
                 ;

              value = value / float_name(0.5226 * 2) + float_name(0.5);
