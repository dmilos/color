             ...
             static component_return_const_type get( input_const_type container, index_input_const_type index )
              {
               switch( index )
                {
                 case( 0 ): return ( ( container >> (                        0 ) ) & ( ( 1 <<  first_size )-1 ) );
                 case( 1 ): return ( ( container >> ( first_size               ) ) & ( ( 1 << second_size )-1 ) );
                 case( 2 ): return ( ( container >> ( first_size + second_size ) ) & ( ( 1 <<  third_size )-1 ) );
                }
               return 0;
              }
             ...
             static set_return_type set( input_type container, index_input_const_type index, component_input_const_type value )
              {
               switch( index )
                {
                 case( 0 ) : container = ( container & ~(  ( ( 1 <<  first_size )-1 ) << (                        0 ) ) )  |  ( ((instance_type)value) << (                        0 ) ); break;
                 case( 1 ) : container = ( container & ~(  ( ( 1 << second_size )-1 ) << ( first_size               ) ) )  |  ( ((instance_type)value) << ( first_size               ) ); break;
                 case( 2 ) : container = ( container & ~(  ( ( 1 <<  third_size )-1 ) << ( first_size + second_size ) ) )  |  ( ((instance_type)value) << ( first_size + second_size ) ); break;
                }
              }
